...
export const Cart: FC<Props> = ({
  ...
  useEffect(() => {
    if (cartRef.current) {
      const wrapperTransformParent = cartRef.current.closest<HTMLElement>(".brz-wrapper-transform");

      if (wrapperTransformParent) {
        // Prevent the parent from being transformed, because this will cause the position: fixed to be relative to the transformed parent
        wrapperTransformParent.style.willChange = "unset";
      }
    }
  }, []);
  ...
  return (
    <div className="third-party-cart" ref={cartRef}>
